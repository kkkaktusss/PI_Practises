...
def task1_wrapper():
    ...
    for i, release_id in enumerate(random.sample(read_json_data('release_ids.json')['releases'], 50)):
        ...
        print(f'Average release "{release_data["release-group"]["title"]}" length {avg_len}')
...
def visualize_lens(release_lens):
    ...
    my_colors = ["#FF5733", "#33FF57", "#3357FF", "#FFFF33"]
    sns.barplot(x="Average length", y="Release", data=df.sort_values(
        by='Average length', ascending=False), palette="viridis")
    plt.show()
...
def visualize_artist(artist_releases):
    ...
    sns.barplot(x='Releases', y='Artist', data=df, palette="viridis")
    plt.show()
